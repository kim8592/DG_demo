...
// ============================================
// MAIN APP COMPONENT - WITH USER ISOLATION
// ============================================
const App = () => {
  // Auth & UI
  ...
  // Data - All tied to user.uid for isolation
  ...
  // ===== CHANGED: Load data per user with user.uid isolation =====
  useEffect(() => {
    if (!user || !isAuthValid) return;
    try {
      // Each teacher has their own data under /teachers/{uid}/data/*
      const unsubYears = db.collection('teachers').doc(user.uid).collection('data').collection('years')
        .onSnapshot(s => setYears(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
      
      const unsubCla = db.collection('teachers').doc(user.uid).collection('data').collection('classes')
        .onSnapshot(s => setClasses(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => a.name.localeCompare(b.name, 'vi', { numeric: true }))));
      
      const unsubMon = db.collection('teachers').doc(user.uid).collection('data').collection('months')
        .onSnapshot(s => setMonths(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
      
      const unsubSub = db.collection('teachers').doc(user.uid).collection('data').collection('subjects')
  ...
  // ===== CHANGED: Load students per user =====
  useEffect(() => {
    ...
    try {
      return db.collection('teachers').doc(user.uid).collection('data').collection('years').doc(selectedYearId)
  ...
  // ===== CHANGED: Load student data per user =====
  useEffect(() => {
    ...
    try {
      return db.collection('teachers').doc(user.uid).collection('data').collection('comments').doc(key)
  ...
  const handleUpdateStatus = useCallback(async (studentId, status) => {
    ...
    try {
      await db.collection('teachers').doc(user.uid).collection('data').collection('years').doc(selectedYearId)
  ...
  }, [user, selectedYearId, selectedClassId, showToast]);
  ...
  const handleMoveStudentToClass = useCallback(async (targetClassId) => {
    ...
    try {
      ...
      const newStudentRef = db.collection('teachers').doc(user.uid).collection('data').collection('years').doc(selectedYearId)
      ...
      const oldStudentRef = db.collection('teachers').doc(user.uid).collection('data').collection('years').doc(selectedYearId)
  ...
  const handleDrop = useCallback(async (e, targetStudentId) => {
    ...
    try {
      ...
      newList.forEach((stu, i) => {
        const ref = db.collection('teachers').doc(user.uid).collection('data').collection('years').doc(selectedYearId)
  ...
  }, [draggedStudentId, students, user, selectedYearId, selectedClassId, showToast]);
  ...
  const handleSaveAllToFirebase = useCallback(async () => {
    ...
    try {
      ...
      const docRefCheck = db.collection('teachers').doc(user.uid).collection('data').collection('comments').doc(key);
      ...
      for (const sId of studentIds) {
        const docRef = db.collection('teachers').doc(user.uid).collection('data').collection('comments').doc(key)
        ...
        if (viewMode === 'subject' && updates.level !== undefined) {
          ...
          if (targetCompId) {
            ...
            const compDocRef = db.collection('teachers').doc(user.uid).collection('data').collection('comments').doc(compKey)
              .collection('entries').doc(sId);
              ...
              {!isSaving && <button onClick={async () => {
                try {
                  ...
                  if (modalType === 'student') {
                    ...
                    const col = db.collection('teachers').doc(user.uid).collection('data').collection('years').doc(selectedYearId)
                      .collection('classes').doc(selectedClassId).collection('students');
                    ...
                    const col = db.collection('teachers').doc(user.uid).collection('data').collection(colName);
                    if (editItem) {
              ...
              <button onClick={async () => {
                try {
                  if (confirmDelete.type === 'student') {
                    ...
                    await db.collection('teachers').doc(user.uid).collection('data').collection(colName).doc(confirmDelete.id).delete();
                  }
