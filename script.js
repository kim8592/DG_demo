...
const batch = db.batch();
  ...
  subjectGuidance['Tiếng Việt'] = 'KIẾN THỨC/KỸ NĂNG: Đọc lưu loát, hiểu ý chính, đọc to, giọng đúng. Viết được câu hoàn chỉnh với chủ ngữ, vị ngữ, tân ngữ. VÍ DỤ: Em đọc bài thơ lưu lotas, phát âm đúng, hiểu ý tình của tác phẩm.';
  subjectGuidance['Toán'] = 'KIẾN THỨC/KỸ NĂNG: Tính toán cộng, trừ, nhân, chia chính xác. Giải toán: hiểu đề, tìm cách giải, trình bày hợp lý. VÍ DỤ: Em giải được bài toán có hai phép tính, trình bày rõ ràng từng bước.';
  subjectGuidance['Khoa học'] = 'KIẾN THỨC/KỸ NĂNG: Quan sát, khám phá, tò mò, hỏi các câu hóc búa. Thí nghiệm thực hiện theo hướng dẫn, ghi chép kết quả. VÍ DỤ: Em tham gia tích cực vào hoạt động thí nghiệm, ghi chép kỹ lưỡng kết quả.';
  subjectGuidance['Thể dục'] = 'KIẾN THỨC/KỸ NĂNG: Kỹ năng chạy, nhảy, bắt bóng, theo quy tắc. Tham gia đầy đủ, không sợ khó. VÍ DỤ: Em chạy nhanh, tham gia tích cực vào trò chơi, biết bảo vệ bản thân.';
  subjectGuidance['Mĩ thuật'] = 'KIẾN THỨC/KỸ NĂNG: Vẽ đúng tỉ lệ, màu sắc hài hòa. Sáng tạo tự tạo các ý tưởng, độc đáo. VÍ DỤ: Em vẽ được các hình vật với tỉ lệ hợp lý, tô màu đẹp mắt.';
// ✅ ĐỊNH NGHĨA systemPromptRetry CHO PHẦN RETRY
  const systemPromptRetry = `
Bạn là giáo viên Tiểu học tại Việt Nam.
Viết nhận xét cụ thể, dễ hiểu, nêu rõ kỹ năng học sinh đã làm được.
...
- KHÔNG có markdown code fence (\`\`\`json)
- KHÔNG có text trước hoặc sau JSON
- JSON phải là object {} với đầy đủ dấu ngoặc kép
`;
...
Hãy viết nhận xét cho từng học sinh theo đúng ID và trả về đúng ${studentContexts.length} nhận xét.`;
  ...
  // Tìm JSON object bên trong text
...
3. Viết tự nhiên, cụ thể, dễ hiểu`;
        ...
        const resRetry = await fetch("https://api.groq.com/openai/v1/chat/completions", {
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "Authorization": `Bearer ${apiKey}`
  ...
  body: JSON.stringify({
    model: "llama-3.3-70b-versatile",
    messages: [
      { role: "system", content: systemPromptRetry },  // ✅ ĐỔI ĐÂY
      { role: "user", content: userInstruction }
    ],
    temperature: 0.7
  })
        ...
        if (!resultRetry.error) {
          const jsonRetry = resultRetry.choices?.[0]?.message?.content;
          if (jsonRetry) {
  let cleanRetry = jsonRetry.trim();
  
  // Xóa markdown code fence nếu có
  cleanRetry = cleanRetry.replace(/^```(?:json)?\n?/, "").replace(/\n?```$/, "").trim();
  
  // Tìm JSON object bên trong text
  const jsonMatch = cleanRetry.match(/\{[\s\S]*\}/);
  if (jsonMatch) {
    cleanRetry = jsonMatch[0];
  }
