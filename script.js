...
const App = () => {
  ...
  const runAI = async () => {
    ...
    try {
      const studentList = allTargets.map(stu => {
        ...
        return `${stu.id}|${stu.name}|${info}${note ? '|GHI CHÚ:' + note : ''}`;
      }).join('\n');

      const systemPrompt = `Bạn là giáo viên Tiểu học Việt Nam, viết nhận xét học sinh.

HƯỚNG DẪN VIẾT:
- Viết cho học sinh, phụ huynh dễ hiểu
- Bắt đầu bằng "Em"
- KHÔNG dùng "cô/thầy/giáo viên", không nêu tên môn học
- **QUAN TRỌNG: KHÔNG ĐƯỢC NÊU LẠI TÊN HỌC SINH TRONG NHẬN XÉT**
- Mỗi nhận xét KHÁC NHAU hoàn toàn
- Viết cụ thể kỹ năng/kiến thức
- Nếu có ghi chú riêng, hãy kết hợp ghi chú vào nhận xét

MỨC T: Khen rõ ràng, KHÔNG nêu cải thiện
MỨC H/Đ: Khen + phải nêu hướng phát huy
MỨC C: Nêu vấn đề + hướng khắc phục

Trả về: [ID]|||[NHẬN XÉT]`;

      const userInstruction = `${aiPrompt ? `Hướng dẫn: ${aiPrompt}\n\n` : ''}Danh sách (ID|Tên|Thông tin|Ghi chú nếu có):
${studentList}

Viết nhận xét theo định dạng:
[ID]|||[NHẬN XÉT]
[ID]|||[NHẬN XÉT]

LƯU Ý: Nếu học sinh có ghi chú riêng, hãy kết hợp nội dung ghi chú vào nhận xét để nhận xét phù hợp hơn. **TUYỆT ĐỐI KHÔNG ĐƯỢC NÊU TÊN HỌC SINH TRONG NHẬN XÉT**.`;
      ...
      if (result.error) {
        ...
        console.error('❌ Lỗi Groq:', errorMsg);
        ...
        if (errorMsg.includes("401") || errorMsg.includes("Unauthorized") || errorMsg.includes("Invalid API Key")) {
          ...
          showToast('❌ Lỗi AI: ' + errorMsg, "error", "❌", 5000);
        }
      ...
      if (!responseText) {
        showToast('❌ Không nhận được response từ AI', 'error', '❌', 4000);
        setIsGenerating(false);
        return;
      }
      ...
      // ===== LỌCBỎ TÊN HỌC SINH KHỎI NHẬN XÉT =====
      lines.forEach(line => {
        ...
        if (parts.length >= 2) {
          ...
          if (id && comment && comment.length > 0) {
            // Xóa tên học sinh nếu có
            const student = allTargets.find(s => s.id === id);
            if (student) {
              // Xóa tên học sinh ở đầu (nếu xuất hiện)
      ...
      if (successCount === 0) {
        console.error('Không tìm thấy định dạng [ID]|||[NHẬN XÉT]:', responseText);
        showToast('❌ Không thể parse nhận xét', 'error', '❌', 4000);
        setIsGenerating(false);
        return;
      }
      ...
      console.log(`✅ Thành công: ${successCount}/${allTargets.length}`);
                ...
                <div className="flex-1 w-full text-left">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl pl-12 pr-4 py-4 text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
                      placeholder="VD: Nội dung cần nhận xét | số lượng từ" 
                      value={aiPrompt} 
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white w-full max-w-lg rounded-3xl p-8 shadow-2xl border-t-8 border-emerald-600">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h3 className="text-xl font-black uppercase text-slate-800">Groq API Key</h3>
            </div>
            <p className="text-sm text-slate-600 mb-4">Lấy API Key tại: <a href="https://console.groq.com/keys" target="_blank" className="text-blue-600 underline">https://console.groq.com/keys</a></p>
            <input type="password" className="w-full p-4 bg-slate-50 border-2 border-slate-300 rounded-xl mb-6 font-mono text-sm outline-none" placeholder="Nhập Groq API Key..." value={apiKey} onChange={e => setApiKey(e.target.value)} />
