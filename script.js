...
const App = () => {
  ...
  // ===== LOAD DATA - Multi-tenant: Each user has isolated data =====
  useEffect(() => {
    if (!user || !isAuthValid) return;
    try {
      const unsubYears = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years')
        .onSnapshot(s => setYears(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
      
      const unsubCla = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('classes')
        .onSnapshot(s => setClasses(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => a.name.localeCompare(b.name, 'vi', { numeric: true }))));
      
      const unsubMon = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('months')
        .onSnapshot(s => setMonths(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
      
      const unsubSub = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('subjects')
  ...
  // ===== LOAD STUDENTS - Multi-tenant path =====
  useEffect(() => {
    ...
    try {
      return db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years').doc(selectedYearId)
  ...
  // ===== LOAD STUDENT DATA - Multi-tenant path =====
  useEffect(() => {
    ...
    try {
      return db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('comments').doc(key)
  ...
  const handleUpdateStatus = async (studentId, status) => {
    ...
    try {
      await db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years').doc(selectedYearId)
  ...
  const handleMoveStudentToClass = async (targetClassId) => {
    ...
    try {
      ...
      const newStudentRef = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years').doc(selectedYearId)
      ...
      const oldStudentRef = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years').doc(selectedYearId)
  ...
  const handleDrop = async (e, targetStudentId) => {
    ...
    try {
      ...
      newList.forEach((stu, i) => {
        const ref = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years').doc(selectedYearId)
  ...
  const handleSaveAllToFirebase = async () => {
    ...
    try {
      ...
      const docRefCheck = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('comments').doc(key);
      ...
      if (!entriesSnap.empty) {
        ...
        if (first.owner && first.owner !== user.uid) {
          const ok = window.confirm(`⚠️ Dữ liệu này đang do "${first.ownerName || 'người khác'}" phụ trách.\n\nBạn có muốn nhận quyền để chỉnh sửa không?`);
          if (!ok) {
            setIsSaving(false);
            return;
          }
      ...
      for (const sId of studentIds) {
        const docRef = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('comments').doc(key)
        ...
        if (viewMode === 'subject' && updates.level !== undefined) {
          ...
          if (targetCompId) {
            ...
            const compDocRef = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('comments').doc(compKey)
              .collection('entries').doc(sId);
              ...
              <button onClick={async () => {
                try {
                  if (modalType === 'student') {
                    ...
                    const col = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('years').doc(selectedYearId)
                      .collection('classes').doc(selectedClassId).collection('students');
                    ...
                    const col = db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection(colName);
                    if (editItem) {
                      ...
                      showToast('Thêm m��i thành công', 'success', '✅', 3000);
                    }
                  }
                  setInputValue(''); 
                  setBulkInput(''); 
              ...
              <button onClick={async () => {
                try {
                  if (confirmDelete.type === 'student') {
                    ...
                    await db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection(colName).doc(confirmDelete.id).delete();
                  }
