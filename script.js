...
const App = () => {
  ...
  const runAI = async () => {
    ...
    try {
      // ===== TẠO DANH SÁCH HỌC SINH DƯỚI DẠNG TEXT (CÓ GHI CHÚ) =====
      const studentList = allTargets.map(stu => {
        ...
        if (viewMode === 'subject') {
          ...
          const lv = draft.level !== undefined ? draft.level : (d.level || "");
          info = `Ti��u chí:${criteriaName}|Mức:${lv}`;
        } else {
          const list = viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES);
...
LƯU Ý: Nếu học sinh có ghi chú riêng, hãy kết hợp nội dung ghi chú vào nhận xét để nhận xét phù hợp hơn.`;

      console.log('📢 Gọi AI một lần cho tất cả học sinh...');
      ...
      lines.forEach(line => {
        const [id, comment] = line.split('|||').map(s => s.trim());
        if (id && comment && comment.length > 0) {
          updates[id] = filterComments(comment);
          successCount++;
        }
      });
                ...
                <div className="flex-1 w-full text-left">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl pl-12 pr-4 py-4 text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
                      placeholder="VD: Khoảng 20 từ | Ngắn gọn | Chi tiết" 
                      value={aiPrompt} 
