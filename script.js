  ...
  <div className="bg-white p-4 rounded-xl border-2 border-slate-300 shadow-sm flex flex-col gap-2 flex-1 min-w-[200px]">
    <div className="flex items-center justify-between mb-1">
      ...
      <div className="flex gap-1">
        {/* ✅ Không hiển thị nút sửa/xóa cho 'month' và 'subject' */}
        ...
        {/* ✅ Không hiển thị nút thêm cho 'month' và 'subject' */}
...
const App = () => {
  ...
  // ✅ MODIFIED: Load years only for current user
  useEffect(() => {
    ...
    try {
      const unsubYears = db.collection('artifacts')
        .doc(appId)
        .collection('users')
        .doc(user.uid) // ✅ Filter by current user
        .collection('years')
        .onSnapshot((s) => {
          setYears(
      ...
      // ✅ MODIFIED: Load classes only for current user
      ...
      // ✅ Months and subjects remain in public (shared)
  ...
  // ✅ MODIFIED: Load students only for current user's class
  useEffect(() => {
  ...
  // ✅ MODIFIED: Load student comments only for current user
  useEffect(() => {
  ...
  const handleMoveStudentToClass = useCallback(async (targetClassId) => {
    ...
    try {
      ...
      // ✅ Filter by current user when creating new student doc
      ...
      // ✅ Filter by current user when deleting old student doc
  ...
  const handleSaveAllToFirebase = useCallback(async () => {
    ...
    try {
      ...
      // ✅ Filter by current user when checking ownership
      ...
      for (const sId of studentIds) {
        // ✅ Filter by current user when saving comments
        ...
        if (viewMode === 'subject' && updates.level !== undefined) {
          ...
          if (targetCompId) {
            ...
            // ✅ Filter by current user when syncing competencies
...
/>
              ...
              {!isSaving && <button onClick={async () => {
                try {
                  ...
                  if (modalType === 'student') {
                    ...
                    // ✅ Add students only to current user's class
  ...
  // ✅ Không cho phép thêm/sửa month và subject
  ...
  const colName =
    ...
    'subjects';
                    ...
                    // ✅ Years and classes are user-specific
                    if (colName === 'classes' || colName === 'years') {
                      ...
                      // Months and subjects remain in public
                      col = db.collection('artifacts')
                        .doc(appId)
                        .collection('public')
                        .doc('data')
              ...
              <button onClick={async () => {
                try {
                  if (confirmDelete.type === 'student') {
                    // ✅ Delete only from current user's class
                    await db.collection('artifacts')
                      .doc(appId)
                      .collection('users')
                      .doc(user.uid)
