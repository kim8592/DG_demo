...
const SPECIFIC_COMPETENCIES = [
  { id: 'lang', name: 'Ngôn ngữ', description: 'đọc, viết, nói tiếng Việt' },
  { id: 'math', name: 'Tính toán', description: 'tính toán số học, giải toán' },
  { id: 'sci', name: 'Khoa học', description: 'hiểu biết về khoa học tự nhiên' },
  { id: 'tech', name: 'Công nghệ', description: 'kỹ năng công nghệ, thực hành' },
  { id: 'it', name: 'Tin học', description: 'kỹ năng tin học, máy tính' },
  { id: 'art', name: 'Thẩm mĩ', description: 'kỹ năng mĩ thuật, âm nhạc' },
  { id: 'phys', name: 'Thể chất', description: 'kỹ năng thể chất, vận động' }
];
...
const processComment = (comment, level) => {
  ...
  comment = comment
    ...
    .replace(/(?:co|thầy|gi��o\s+viên)\s+(?:nhận\s+xét|khen)/gi, '')
  ...
  if (level === "T") {
    if (hasDevelopment) {
      comment = comment.replace(/[\s,.]*(nhưng|tuy nhiên|song|cần|nên|thêm|tiếp tục|chú ý|cố gắng|khắc phục|rèn luyện).*/i, "").trim();
    ...
    if (!/đáng khen|đáng ghi nhận|xuất sắc|nổi bật/i.test(comment) || comment.length < 65) {
      const praiseEnds = ["thể hiện rất tốt, đáng khen!", "có nhiều cố gắng đáng ghi nhận.", "cho thấy sự chăm chỉ rõ rệt.", "là tấm gương học tập tốt.", "xứng đáng được tuyên dương!", "đem lại kết quả học tập tích cực.", "thể hiện tinh thần học tập rất tốt.", "có ý thức học tập đáng biểu dương!", "đạt kết quả nổi bật trong học tập.", "có nhiều nỗ lực rất đáng khen!"];
    ...
    comment = comment.replace(/,\s*tuy nhiên,?\s*/gi, ". ").replace(/\.\s*tuy nhiên,?\s*/gi, ". ").replace(/\s+tuy nhiên,?\s*/gi, ". ");
    comment = comment.replace(/(\.|,|\s)cần\s(?=rèn|phát huy|chú ý|luyện|cố gắng)/gi, "$1tiếp tục ");
    comment = comment.replace(/tiếp tục\s+tiếp tục/gi, "tiếp tục");
    comment = comment.replace(/Tiếp tục\s+ti��p tục/gi, "Tiếp tục");

    if (!/tiếp tục|phát huy|rèn luyện|cố gắng|duy trì|nâng cao/i.test(comment)) {
      const encourages = ["Em tiếp tục phát huy nhé.", "Em tiếp tục cố gắng để tiến bộ hơn.", "Em duy trì tinh thần học tập tích cực nhé.", "Em tiếp tục rèn luyện để đạt kết quả tốt hơn.", "Em cố gắng thêm để ngày càng tiến bộ.", "Em phát huy khả năng của mình hơn nữa."];
      comment = comment.replace(/[.!?]\s*$/, "");
      const endText = encourages[Math.floor(Math.random() * encourages.length)];
      comment += ". " + endText;
    }

    comment = comment.replace(/([.!?]\s+)([a-zà-ỹ])/g, (m, p1, p2) => p1 + p2.toUpperCase());
    ...
    if (!/cần|nên|chú ý|rèn luyện|cố gắng/i.test(comment)) {
      const supports = ["Em cần luyện tập thêm để cải thiện kỹ năng.", "Em nên rèn luyện thêm để nắm vững kiến thức.", "Em cần chú ý hơn trong quá trình làm bài.", "Em nên luyện tập thường xuyên hơn để tiến bộ.", "Em cố gắng thêm để hoàn thiện bài làm tốt hơn."];
      comment = comment.replace(/[.!?]\s*$/, "");
      const endText = supports[Math.floor(Math.random() * supports.length)];
      comment += ". " + endText;
    }
...
const App = () => {
  ...
  // LOAD YEARS, CLASSES, MONTHS, SUBJECTS - FLAT STRUCTURE
  useEffect(() => {
    if (!user || !isAuthValid) return;
    try {
      const unsubYears = db.collection('teachers').doc(user.uid).collection('years')
        .onSnapshot(s => setYears(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
      
      const unsubCla = db.collection('teachers').doc(user.uid).collection('classes')
        .onSnapshot(s => setClasses(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => a.name.localeCompare(b.name, 'vi', { numeric: true }))));
      
      const unsubMon = db.collection('teachers').doc(user.uid).collection('months')
        .onSnapshot(s => setMonths(s.docs.map(d => ({id: d.id, ...d.data()})).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))));
      
      const unsubSub = db.collection('teachers').doc(user.uid).collection('subjects')
      ...
      showToast('Lỗi tải dữ liệu: ' + e.message, 'error', '❌', 4000);
    }
  }, [user, isAuthValid, showToast]);
  ...
  // LOAD STUDENTS - QUERY WITH WHERE CLAUSES
  useEffect(() => {
    ...
    try {
      return db.collection('teachers').doc(user.uid).collection('students')
        ...
        .onSnapshot((snap) => {
          ...
          showToast('Lỗi tải học sinh: ' + e.message, 'error', '❌', 4000);
        });
    } catch (e) {
      console.error('Students subscription error:', e);
      showToast('Lỗi kết nối: ' + e.message, 'error', '❌', 4000);
    }
  }, [user, selectedClassId, selectedYearId, isAuthValid, showToast]);

  // LOAD STUDENT DATA / COMMENTS
  useEffect(() => {
    ...
    try {
      return db.collection('teachers').doc(user.uid).collection('comments').doc(key)
      ...
      console.error('Student data subscription error:', e);
    }
  ...
  const handleUpdateStatus = useCallback(async (studentId, status) => {
    ...
    try {
      await db.collection('teachers').doc(user.uid).collection('students').doc(studentId)
        .update({ status });
  ...
  const handleMoveStudentToClass = useCallback(async (targetClassId) => {
    ...
    try {
      ...
      const newStudentId = db.collection('teachers').doc(user.uid).collection('students').doc().id;
      const newStudentRef = db.collection('teachers').doc(user.uid).collection('students').doc(newStudentId);
      ...
      const oldStudentRef = db.collection('teachers').doc(user.uid).collection('students').doc(statusModalStudent.id);
  ...
  }, [statusModalStudent, user, selectedYearId, selectedClassId, isMoving, showToast]);
  ...
  const handleDrop = useCallback(async (e, targetStudentId) => {
    ...
    try {
      ...
      newList.forEach((stu, i) => {
        const ref = db.collection('teachers').doc(user.uid).collection('students').doc(stu.id);
  ...
  }, [draggedStudentId, students, user, showToast]);
  ...
  const handleSaveAllToFirebase = useCallback(async () => {
    ...
    try {
      ...
      const docRefCheck = db.collection('teachers').doc(user.uid).collection('comments').doc(key);
      ...
      if (!entriesSnap.empty) {
        ...
        if (first.owner && first.owner !== user.uid) {
          const ok = window.confirm(`⚠️ Dữ liệu này đang do "${first.ownerName || 'giáo viên khác'}" phụ trách.\n\nBạn có muốn chỉnh sửa không?`);
          if (!ok) {
            setIsSaving(false);
            return;
          }
      ...
      for (const sId of studentIds) {
        const docRef = db.collection('teachers').doc(user.uid).collection('comments').doc(key)
        ...
        if (viewMode === 'subject' && updates.level !== undefined) {
          ...
          if (targetCompId) {
            ...
            const compDocRef = db.collection('teachers').doc(user.uid).collection('comments').doc(compKey)
              .collection('entries').doc(sId);
  ...
  const runAI = useCallback(async () => {
    ...
    // Special handling for SMAS specific competencies
    ...
    const allTargets = students.filter(s => {
      ...
      if (isSmaSSpecific) {
        // For SMAS specific: check if any of the 7 competencies has a level set
        return SPECIFIC_COMPETENCIES.some(c => {
        ...
        if (viewMode === 'subject') {
          ...
          const list = viewMode === 'quality' ? QUALITY_CRITERIA : GENERAL_COMPETENCIES;
          return list.some(c => {
            const level = draft[`level_${c.id}`] !== undefined ? draft[`level_${c.id}`] : (d[`level_${c.id}`] || "");
            return !!level;
          });
    ...
    try {
      ...
      if (isSmaSSpecific) {
        // Build competency data for SMAS specific
        allTargets.forEach((stu, idx) => {
          ...
          let compInfo = SPECIFIC_COMPETENCIES.map(c => {
          ...
          studentListText += `${idx + 1}. ${stu.name} | ${compInfo}${noteText}\n`;
        });

        const userInstruction = AI_PROMPTS.specificCompetenciesUser(studentListText, aiPrompt, "Đánh giá 7 năng lực: Ngôn ngữ, Tính toán, Khoa học, Công nghệ, Tin học, Thẩm mĩ, Thể chất");
        ...
        lines.forEach(line => {
          ...
          if (parts.length >= 2) {
            ...
            if (student && comment && comment.length > 10) {
              ...
              // Determine level from SMAS specific competencies
  ...
  const handleSendZalo = useCallback((studentId) => {
    ...
    if (systemMode === 'smas' && viewMode === 'specific') {
      // Get level from first competency with a value
      const d = studentData[studentId] || {};
      const draft = draftData[studentId] || {};
      for (const comp of SPECIFIC_COMPETENCIES) {
        const lv = draft[`level_${comp.id}`] !== undefined ? draft[`level_${comp.id}`] : (d[`level_${comp.id}`] || "");
        if (lv) {
          levelTxt = lv;
          break;
        }
      }
      label = 'Năng lực - Phẩm chất';
    } else if (systemMode === 'vnedu' && viewMode !== 'subject') {
      const list = viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES);
      label = list.find(c => c.id === selectedCriteriaId)?.name;
      levelTxt = draft.level !== undefined ? draft.level : (d.level || "Đ");
    } else {
      label = viewMode === 'subject' ? subjects.find(s => s.id === selectedSubId)?.name : 'Năng lực - Phẩm chất';
      levelTxt = draft.level !== undefined ? draft.level : (d.level || "Đ");
    }
              ...
              <div className="flex flex-col xl:flex-row gap-3 items-end">
                <div className="flex-1 w-full text-left">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl pl-12 pr-4 py-4 text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
                      placeholder={systemMode === 'smas' && viewMode === 'specific' ? "VD: kỹ năng giao tiếp tốt, tính toán nhanh, vận động linh hoạt... khoảng 20 từ" : "VD: hãy random nội dung sau: biết cộng có nhớ, biết giải toán, nắm vững hình học, biết cách tìm tổng. khoảng 20 từ "} 
                      value={aiPrompt} 
                ...
                <div className="flex gap-3 w-full xl:w-auto">
                  <button onClick={runAI} disabled={isGenerating || students.length === 0} className={`flex-1 xl:flex-none px-10 py-4 ${isGenerating ? 'bg-gray-500' : 'bg-indigo-600 hover:bg-indigo-700'} text-white rounded-xl font-black text-xs flex items-center justify-center gap-2 uppercase shadow-lg transition-all active:scale-95 disabled:opacity-50`}>
                   ...
                   <button onClick={() => { setEditItem(null); setBulkInput(''); setModalType('student'); }} className="p-4 bg-white text-slate-800 border border-gray-200 rounded-xl shadow-lg hover:bg-gray-100 transition-all active:scale-95 text-lg">👥</button>
                </div>
              </div>
              ...
              {!isSaving && <button onClick={async () => {
                try {
                  ...
                  if (modalType === 'student') {
                    ...
                    const col = db.collection('teachers').doc(user.uid).collection('students');
                    ...
                    const col = db.collection('teachers').doc(user.uid).collection(colName);
                    if (editItem) {
              ...
              <button onClick={async () => {
                try {
                  if (confirmDelete.type === 'student') {
                    await db.collection('teachers').doc(user.uid).collection('students').doc(confirmDelete.id).delete();
                  } else {
                    const colName = confirmDelete.type === 'year' ? 'years' : confirmDelete.type === 'class' ? 'classes' : confirmDelete.type === 'month' ? 'months' : 'subjects';
                    await db.collection('teachers').doc(user.uid).collection(colName).doc(confirmDelete.id).delete();
                  }
