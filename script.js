...
QUAN TRỌNG: Hãy tạo nhận xét cho TẤT CẢ học sinh trong danh sách, không bỏ sót em nào.`
};
  ...
  <div className="bg-white p-4 rounded-xl border-2 border-slate-300 shadow-sm flex flex-col gap-2 flex-1 min-w-[200px]">
    <div className="flex items-center justify-between mb-1">
      ...
      <div className="flex gap-1">
        {selectedId && !['system_comp', 'month', 'subject'].includes(type) && (
          <>
        ...
        {!['system_comp', 'month', 'subject'].includes(type) && <button onClick={onAdd} className="p-1 text-indigo-500 text-xs">➕</button>}
      </div>
    </div>
...
const App = () => {
  ...
  }, [user, statusModalStudent, selectedYearId, selectedClassId, isMoving, showToast]);
  ...
  }, [user, draggedStudentId, students, selectedYearId, selectedClassId, showToast]);
  ...
  }, [user, draftData, systemMode, viewMode, selectedYearId, selectedSubId, selectedMonthId, selectedClassId, selectedCriteriaId, subjects, showToast]);

  // ===== AI GENERATION - FIX CHÍNH =====
  const runAI = useCallback(async () => {
    ...
    // ✅ FIX: Lấy TẤT CẢ học sinh đang học, không filter chỉ những em chưa có comment
    const allTargets = students.filter(s => {
      if (s.status !== 'active') return false;
      
      if (viewMode === 'subject') {
        const level = draftData[s.id]?.level !== undefined ? draftData[s.id].level : (studentData[s.id]?.level || "");
        return !!level;
        ...
        return list.some(c => {
          const level = draftData[s.id]?.[`level_${c.id}`] !== undefined ? draftData[s.id][`level_${c.id}`] : (studentData[s.id]?.[`level_${c.id}`] || "");
          return !!level;
        });
      }
      return false;
    ...
    try {
      ...
      console.log('📤 Sending to AI:', userInstruction); // ✅ DEBUG
      ...
      const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent?key=${apiKey}`, {
        ...
        body: JSON.stringify({ 
          ...
          generationConfig: { 
            temperature: 0.3,
            maxOutputTokens: 3000  // ✅ TĂNG: từ 2000 -> 3000 để đủ token cho nhiều em
          }
        })
      });
      ...
      // ✅ FIX: Tìm tất cả dòng có format [Name]|||[Comment]
      ...
      lines.forEach(line => {
        ...
        if (parts.length >= 2) {
          const nameOrId = parts[0].trim().replace(/^\d+\.\s*/, ''); // ✅ Bỏ số thứ tự
          let comment = parts.slice(1).join('|||').trim();
          
          // ✅ FIX: Tìm kiếm linh hoạt hơn
          let student = allTargets.find(s => 
            s.name.toLowerCase() === nameOrId.toLowerCase() ||
            s.id === nameOrId ||
            s.name.toLowerCase().includes(nameOrId.toLowerCase()) ||
            nameOrId.toLowerCase().includes(s.name.toLowerCase())
          );
      ...
      if (successCount === 0) {
        ...
        console.error('Failed to find students:', failedNames);
        showToast(`Lỗi: Không tìm được học sinh. Kiểm tra tên: ${failedNames.slice(0, 3).join(', ')}...`, 'error', '❌', 5000);
        setIsGenerating(false);
        return;
      }

      // ✅ FIX: Cập nhật TẤT CẢ bản nháp, không chỉ những em có comment
      setDraftData(prev => {
        const updated = { ...prev };
        // Giữ lại tất cả draft cũ
              ...
              {!isSaving && <button onClick={async () => {
                try {
                  ...
                  if (modalType === 'student') {
                    ...
                    if (['month', 'subject'].includes(modalType)) {
                      showToast('Không được phép sửa ' + (modalType === 'month' ? 'tháng' : 'môn học'), 'error', '❌', 3000);
                      setIsSaving(false);
                      return;
                    }
              ...
              <button onClick={async () => {
                try {
                  if (confirmDelete.type === 'student') {
                    ...
                    if (['month', 'subject'].includes(confirmDelete.type)) {
                      showToast('Không được phép xóa ' + (confirmDelete.type === 'month' ? 'tháng' : 'môn học'), 'error', '❌', 4000);
                      return;
                    }
